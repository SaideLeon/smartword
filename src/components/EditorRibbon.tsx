...
export function EditorRibbon({ editor, activeTab }: Props) {
  ...
  // Grupo com label — shrink-0 impede que encolha no scroll horizontal
  const Group = ({ children, label, minWidth = 'auto' }: {
    children: React.ReactNode; label: string; minWidth?: string | number;
  }) => (
    <div
      className="relative flex shrink-0 flex-col items-center px-2"
      style={{ minWidth }}
    >
  ...
  return (
    <>
      ...
      {/*
        ── Estrutura de scroll horizontal ────────────────────────────────────
        O div externo define a altura e esconde a scrollbar.
        O div interno (w-max) expande até ao conteúdo real — é ele que permite
        o scroll sem comprimir os grupos. Sem este wrapper, os grupos encolhem.
      */}
      <div
        className="h-[72px] shrink-0 overflow-x-auto border-b border-[var(--border)] bg-[var(--surface)]"
        style={{ scrollbarWidth: 'none', WebkitOverflowScrolling: 'touch' }}
      >
        {/* w-max → largura = soma de todos os grupos, nunca menos */}
        <div className="flex h-full w-max items-stretch px-1">
          ...
          <Group label="Área de transferência" minWidth={86}>
            <BigBtn
              ...
              label="Colar"
              onClick={() => { try { document.execCommand('paste'); } catch {} editor.commands.focus(); }}
              title="Colar (Ctrl+V)"
            />
            <div className="flex flex-col gap-0.5">
              <Btn onClick={() => { try { document.execCommand('cut'); } catch {} }} title="Cortar">
                <Scissors className="h-3.5 w-3.5" /><span>Cortar</span>
              </Btn>
              <Btn onClick={() => { try { document.execCommand('copy'); } catch {} }} title="Copiar">
                <ClipboardCopy className="h-3.5 w-3.5" /><span>Copiar</span>
              </Btn>
            </div>
          </Group>
          ...
          <Group label="Parágrafo" minWidth={116}>
            <div className="flex flex-col gap-1">
              ...
              <div className="flex gap-0.5">
                <Btn active={isAlignLeft}    onClick={() => editor.chain().focus().setTextAlign('left').run()}    title="Alinhar à esquerda"><AlignLeft    className="h-3.5 w-3.5" /></Btn>
                <Btn active={isAlignCenter}  onClick={() => editor.chain().focus().setTextAlign('center').run()}  title="Centrar">           <AlignCenter  className="h-3.5 w-3.5" /></Btn>
                <Btn active={isAlignRight}   onClick={() => editor.chain().focus().setTextAlign('right').run()}   title="Alinhar à direita"> <AlignRight   className="h-3.5 w-3.5" /></Btn>
                <Btn active={isAlignJustify} onClick={() => editor.chain().focus().setTextAlign('justify').run()} title="Justificar">        <AlignJustify className="h-3.5 w-3.5" /></Btn>
          ...
          <div className="flex shrink-0 flex-col items-center px-2" style={{ minWidth: 50 }}>
