...
export function EditorStatusBar({
  ...
  return (
    <div className={`shrink-0 border-t border-[var(--border)] bg-[var(--surface)] px-3 ${isMobile ? 'py-2' : 'flex h-[28px] items-center gap-3'}`}>
      <div className={`flex items-center ${isMobile ? 'justify-between gap-2' : 'gap-3 w-full'}`}>
        {/* Stats */}
        ...
        <button
          ...
          onClick={onExport}
          disabled={loading}
          className={`flex shrink-0 cursor-pointer items-center gap-1.5 rounded-md border-none bg-gradient-to-br from-[var(--gold)] to-[var(--gold2)] px-3 py-1 font-mono text-[10px] font-bold text-black transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-50 ${isMobile ? '' : 'ml-2'}`}
        >
          {loading ? (
            <>
